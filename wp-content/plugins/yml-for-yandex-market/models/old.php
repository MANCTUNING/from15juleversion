...
function yfym_old($postId, $numFeed='1') {	
 yfym_error_log('FEED № '.$numFeed.'; Стартовала yfym_old. $postId = '.$postId.'; Файл: old.php; Строка: '.__LINE__, 0);	
 ...
 if ($product == null) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к get_post вернула null; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}

 if ($product->is_type('grouped')) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к сгруппированный; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
 
 // что выгружать
 if ($product->is_type('variable')) {
	$yfym_whot_export = yfym_optionGET('yfym_whot_export', $numFeed, 'set_arr');
	if ($yfym_whot_export === 'simple') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к вариативный; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
 }
 
 if (get_post_meta($postId, 'yfymp_removefromyml', true) === 'on')  {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен принудительно; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
 $skip_flag = apply_filters('yfym_skip_flag', $skip_flag, $postId, $product, $numFeed); /* c версии 3.2.6 */
 if ($skip_flag === true) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен по флагу; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
 ...
 */	 
 ...
 $params_arr = unserialize(yfym_optionGET('yfym_params_arr'));
 ...
 */	
 ...
 if ($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') {
 ...
 if ($product->is_type('variable')) {
	yfym_error_log('FEED № '.$numFeed.'; У нас вариативный товар. Файл: old.php; Строка: '.__LINE__, 0);	
	...
	for ($i = 0; $i<$variation_count; $i++) {
		...
		*/
		...
		if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет цены; Файл: old.php; Строка: '.__LINE__, 0); continue;}
		...
		if ($yfym_skip_missing_products === 'on') {
			if ($offer->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к ее нет в наличии; Файл: old.php; Строка: '.__LINE__, 0); continue;}
		}
		...
		if ($skip_backorders_products === 'on') {
		 if ($offer->get_manage_stock() == true) { // включено управление запасом
			if (($offer->get_stock_quantity() < 1) && ($offer->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ и включено управление запасом; Файл: old.php; Строка: '.__LINE__, 0); continue;}
		 } else {
			if ($offer->get_stock_status() === 'onbackorder') { // предзаказ на уровнее вариации включен
				yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ на уровне вариации и отключено управление запасом; Файл: old.php; Строка: '.__LINE__, 0); continue;
			}
		 }
		}
		...
		if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {	  
			yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет картинки даже в галерее; Файл: old.php; Строка: '.__LINE__, 0); continue; /*continue;*/  
		}

		$skip_flag = apply_filters('yfym_skip_flag_variable', $skip_flag, $postId, $product, $offer, $numFeed); /* c версии 3.2.6 */
		if ($skip_flag === true) {yfym_error_log('FEED № '.$numFeed.'; Вариативный товар с postId = '.$postId.', offer_id = '.$offer_id.' пропущен по флагу; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
		if ($skip_flag === 'continue') {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с с postId = '.$postId.', offer_id = '.$offer_id.' пропущена по флагу; Файл: old.php; Строка: '.__LINE__, 0); $skip_flag = false; continue;}   
		...
		*/
		...
		if (empty($no_group_id_arr)) {
			...
			if (in_array($CurCategoryId, $no_group_id_arr)) {
			 ...
			 foreach ($attributes as $param) {					
				...
				if ($param_at_name == '') {
					yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар т.к. нет атрибутов вариаций для уникальзации заголовков. Файл: old.php; Строка: '.__LINE__, 0);	
		...
		if (($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') && $result_yml_country_of_origin === '') {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем вариацию offer_id = '.$offer_id.' товара $postId = '.$postId.' т.к. нет country_of_origin. Файл: old.php; Строка: '.__LINE__, 0);	
			continue;		
		} 
		...
		if (($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') && $result_yml_manufacturer === '') {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем вариацию offer_id = '.$offer_id.' товара $postId = '.$postId.' т.к. нет manufacturer. Файл: old.php; Строка: '.__LINE__, 0);	
			continue;			
		} 
		...
		if (($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') && $result_yml_vendor === '') {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем вариацию offer_id = '.$offer_id.' товара $postId = '.$postId.' т.к. нет vendor. Файл: old.php; Строка: '.__LINE__, 0);	
			continue;
		} 			
		...
		if ($yfym_yml_rules === 'beru' && $result_yml_shop_sku === '') {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем вариацию $offer_id = '.$offer_id.' т.к. нет shop-sku. Файл: old.php; Строка: '.__LINE__, 0);	
			continue;
		} 
		...
		if ($yfym_yml_rules === 'beru') {$result_yml .= $result_yml_shop_sku;}
	...
	yfym_error_log('FEED № '.$numFeed.'; Все вариации выгрузили. '.$ids_in_yml.' Файл: old.php; Строка: '.__LINE__, 0);	
	
	return array($result_yml, $ids_in_yml); // все вариации выгрузили	
 } // end if ($product->is_type('variable'))	 
 /* end Вариации */

 yfym_error_log('FEED № '.$numFeed.'; У нас обычный товар. Файл: old.php; Строка: '.__LINE__, 0);
 ...
 if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет цены; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
 if (class_exists('YmlforYandexMarketPro')) {
	if ((yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== false) && (yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== '')) {
		$yfymp_compare_value = yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr');
		$yfymp_compare = yfym_optionGET('yfymp_compare', $numFeed, 'set_arr');			 
 ...
 $yfym_skip_missing_products = yfym_optionGET('yfym_skip_missing_products', $numFeed, 'set_arr');
 yfym_error_log('FEED № '.$numFeed.'; $yfym_skip_missing_products = '.$yfym_skip_missing_products.'; gettype = '.gettype($yfym_skip_missing_products).'; Файл: old.php; Строка: '.__LINE__, 0);
 if ($yfym_skip_missing_products === 'on') {
	if ($product->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет в наличии; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml;}
 }		  
 ...
 if ($skip_backorders_products === 'on') {
	if ($product->get_manage_stock() == true) { // включено управление запасом  
		if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ и включено управление запасом; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	} else {
		if ($product->get_stock_status() !== 'instock') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	}
 }  
 ...
 $yfym_skip_products_without_pic = yfym_optionGET('yfym_skip_products_without_pic', $numFeed, 'set_arr'); 
 if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {	  
	yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет картинки даже в галерее; Файл: old.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/  
 }
 ...
 if (($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') && $result_yml_country_of_origin === '') {
	yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. нет country_of_origin. Файл: old.php; Строка: '.__LINE__, 0);	
	return $result_yml;			
 } 
 ...
 if (($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') && $result_yml_manufacturer === '') {
	yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. нет manufacturer. Файл: old.php; Строка: '.__LINE__, 0);	
	return $result_yml;			
 } 
 ...
 if (($yfym_yml_rules === 'beru' || $yfym_yml_rules === 'single_catalog') && $result_yml_vendor === '') {
	yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. нет vendor. Файл: old.php; Строка: '.__LINE__, 0);	
	return $result_yml;			
 }
 ...
 if ($yfym_yml_rules === 'beru' && $result_yml_shop_sku === '') {
	yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар т.к. нет shop-sku. Файл: old.php; Строка: '.__LINE__, 0);	
	return $result_yml;			
 } 
