...
function yfym_ozon($postId, $product, $data, $numFeed) {
 // https://seller-edu.ozon.ru/docs/work-with-goods/fidi.html
 yfym_error_log('FEED № '.$numFeed.'; Стартовала yfym_ozon. $postId = '.$postId.'; Файл: ozon.php; Строка: '.__LINE__, 0);	
...
// добавить period-of-validity-days, service-life-days
 ...
 $result_premium_price_yml = '';
 if (get_post_meta($postId, '_yfym_premium_price', true) !== '') {
	$yfym_premium_price = get_post_meta($postId, '_yfym_premium_price', true);
	$result_premium_price_yml .= '<premium_price>'.$yfym_premium_price.'</premium_price>'.PHP_EOL;
 }
 ...
 if ($product->is_type('variable')) {
	yfym_error_log('FEED № '.$numFeed.'; У нас вариативный товар. Файл: ozon.php; Строка: '.__LINE__, 0);	
	...
	for ($i = 0; $i<$variation_count; $i++) {
		...
		*/
		...
		if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет цены; Файл: ozon.php; Строка: '.__LINE__, 0); continue;}
		...
		if ($yfym_skip_missing_products === 'on') {
			if ($offer->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к ее нет в наличии; Файл: ozon.php; Строка: '.__LINE__, 0); continue;}
		}
		...
		if ($skip_backorders_products === 'on') {
		 if ($offer->get_manage_stock() == true) { // включено управление запасом
			if (($offer->get_stock_quantity() < 1) && ($offer->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ и включено управление запасом; Файл: ozon.php; Строка: '.__LINE__, 0); continue;}
		 } else {
			if ($offer->get_stock_status() === 'onbackorder') { // предзаказ на уровнее вариации включен
				yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ на уровне вариации и отключено управление запасом; Файл: ozon.php; Строка: '.__LINE__, 0); continue;
			}
		 }
		}
		...
		if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {	  
			yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет картинки даже в галерее; Файл: ozon.php; Строка: '.__LINE__, 0); continue; /*continue;*/  
		}

		$skip_flag = apply_filters('yfym_skip_flag_variable', $skip_flag, $postId, $product, $offer, $numFeed); /* c версии 3.2.6 */
		if ($skip_flag === true) {yfym_error_log('FEED № '.$numFeed.'; Вариативный товар с postId = '.$postId.', offer_id = '.$offer_id.' пропущен по флагу; Файл: ozon.php; Строка: '.__LINE__, 0); return $result_yml;}
		if ($skip_flag === 'continue') {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с с postId = '.$postId.', offer_id = '.$offer_id.' пропущена по флагу; Файл: ozon.php; Строка: '.__LINE__, 0); $skip_flag = false; continue;}   
		...
		if (empty($no_group_id_arr)) {
			...
			if (in_array($CurCategoryId, $no_group_id_arr)) {
			 ...
			 foreach ($attributes as $param) {					
				...
				if ($param_at_name == '') {
					yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар т.к. нет атрибутов вариаций для уникальзации заголовков. Файл: ozon.php; Строка: '.__LINE__, 0);	
		...
		if ($yfym_source_id === 'sku') {
			$offer_id_yml = $offer->get_sku(); // артикул
			if (empty($offer_id_yml)) {
				yfym_error_log('FEED № '.$numFeed.'; Пропускаем вариацию offer_id = '.$offer_id.' товара $postId = '.$postId.' т.к. не задан id для OZON. Файл: ozon.php; Строка: '.__LINE__, 0);	
				continue;
			} 
		}
		switch ($yfym_source_id) { 
			...
			break;					
			case "post_meta": 
				$yfym_source_id_post_meta_id = yfym_optionGET('yfym_source_id_post_meta', $numFeed, 'set_arr');
				$yfym_source_id_post_meta_id = trim($yfym_source_id_post_meta_id);
				if (get_post_meta($postId, $yfym_source_id_post_meta_id, true) !== '') {
					$offer_id_yml = get_post_meta($postId, $yfym_source_id_post_meta_id, true);
				}
			break;	
			case "germanized":
				if (class_exists('WooCommerce_Germanized')) {
					if (get_post_meta($postId, '_ts_gtin', true) !== '') {
						$offer_id_yml = get_post_meta($postId, '_ts_gtin', true);
					}
				}
			break;
			default: $offer_id_yml = $offer_id;
		}
		...
		$result_yml .= '<offer '.$offer_type.$gi.' id="'.$offer_id_yml.'" available="'.$available.'" '.$append_offer_tag.'>'.PHP_EOL;		
...
//			if ($yfym_count === 'enabled' && $stock_quantity > -1) {
				$result_yml .= '<instock>'.$stock_quantity.'</instock>'.PHP_EOL;
//			}
		} else {
...
//				}
		...
		$result_yml .= $result_premium_price_yml;
	...
	yfym_error_log('FEED № '.$numFeed.'; Все вариации выгрузили. '.$ids_in_yml.' Файл: ozon.php; Строка: '.__LINE__, 0);	
	
	return array($result_yml, $ids_in_yml); // все вариации выгрузили	
 } // end if ($product->is_type('variable'))	 
 /* end Вариации */

 yfym_error_log('FEED № '.$numFeed.'; У нас обычный товар. Файл: ozon.php; Строка: '.__LINE__, 0);
 ...
 if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет цены; Файл: ozon.php; Строка: '.__LINE__, 0); return $result_yml;}
 if (class_exists('YmlforYandexMarketPro')) {
	if ((yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== false) && (yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== '')) {
		$yfymp_compare_value = yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr');
		$yfymp_compare = yfym_optionGET('yfymp_compare', $numFeed, 'set_arr');			 
 ...
 $yfym_skip_missing_products = yfym_optionGET('yfym_skip_missing_products', $numFeed, 'set_arr');
 yfym_error_log('FEED № '.$numFeed.'; $yfym_skip_missing_products = '.$yfym_skip_missing_products.'; gettype = '.gettype($yfym_skip_missing_products).'; Файл: ozon.php; Строка: '.__LINE__, 0);
 if ($yfym_skip_missing_products === 'on') {
	if ($product->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет в наличии; Файл: ozon.php; Строка: '.__LINE__, 0); return $result_yml;}
 }		  
 ...
 if ($skip_backorders_products === 'on') {
	if ($product->get_manage_stock() == true) { // включено управление запасом  
		if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ и включено управление запасом; Файл: ozon.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	} else {
		if ($product->get_stock_status() !== 'instock') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ; Файл: ozon.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	}
 }  
 ...
 if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {	  
	yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет картинки даже в галерее; Файл: ozon.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/  
 }
 ...
 switch ($yfym_source_id) { 
	case "sku": 
		$post_id_yml = $product->get_sku(); // артикул
		if (empty($post_id_yml)) {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. не задан id для OZON. Файл: ozon.php; Строка: '.__LINE__, 0);	
			return $result_yml;
		} 
	break;					
	case "post_meta": 
		...
		$yfym_source_id_post_meta_id = trim($yfym_source_id_post_meta_id);
		if (get_post_meta($postId, $yfym_source_id_post_meta_id, true) !== '') {
			$post_id_yml = get_post_meta($postId, $yfym_source_id_post_meta_id, true);
		} else {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. метаполе с id для OZON пустое. Файл: ozon.php; Строка: '.__LINE__, 0);	
			return $result_yml;			
		}
	break;	
	case "germanized":
		if (class_exists('WooCommerce_Germanized')) {
			if (get_post_meta($postId, '_ts_gtin', true) !== '') {
				$post_id_yml = get_post_meta($postId, '_ts_gtin', true);
			} else {
				yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. метаполе WooCommerce_Germanized с id для OZON пустое. Файл: ozon.php; Строка: '.__LINE__, 0);	
				return $result_yml;			
			}
		} else {
			yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар с postId = '.$postId.' т.к. не активен плагин WooCommerce_Germanized и нет источника для id для OZON. Файл: ozon.php; Строка: '.__LINE__, 0);	
			return $result_yml;			
		}
	break;
	default: $post_id_yml = $postId;
 }

 /* с версии 2.1.2 */ 
 $append_offer_tag = apply_filters('yfym_append_offer_tag_filter', $append_offer_tag, $product, $numFeed);
 $result_yml .= '<offer '.$offer_type.' id="'.$post_id_yml.'" available="'.$available.'" '.$append_offer_tag.'>'.PHP_EOL;
