...
class Request
{
    ...
    protected function prepareHeaders($modified = null)
    {
        ...
        if ($this->parameters->getAuth('token')) {
            $headers[] = 'Authorization: Bearer ' . $this->parameters->getAuth('token');
        }
    ...
    protected function prepareEndpoint($url)
    {
        if ($this->parameters->getAuth('token')) {
            return sprintf(
                ...
                $url,
                http_build_query(array_merge($this->parameters->getGet(), []), null, '&')
            );
        }
    ...
    protected function request($url, $modified = null)
    {
        ...
        if ($this->parameters->hasPost()) {
            if (in_array($url, ['/api/v2/leads', '/api/v2/catalog_elements'])) {
