...
class LogHelper
{
    ...
    {
        if (!file_exists(WC_AMOCRM_PLUGIN_LOG_FILE)) {
            header('Content-Type: plain/text');
            header('Content-Disposition: attachment; filename="' . basename(WC_AMOCRM_PLUGIN_LOG_FILE) . '"');
        ...
        if (function_exists('zip_open')) {
            $file = dirname(WC_AMOCRM_PLUGIN_LOG_FILE) . '/' . uniqid() . '.zip';
            ...
            $zip->addFile(WC_AMOCRM_PLUGIN_LOG_FILE, basename(WC_AMOCRM_PLUGIN_LOG_FILE));
        ...
        header('Content-Type: plain/text');
        header('Content-Disposition: attachment; filename="' . basename(WC_AMOCRM_PLUGIN_LOG_FILE) . '"');
        header('Content-Length: ' . filesize(WC_AMOCRM_PLUGIN_LOG_FILE));

        readfile(WC_AMOCRM_PLUGIN_LOG_FILE);
    ...
    {
        if (!file_exists(WC_AMOCRM_PLUGIN_LOG_FILE) || !is_writable(WC_AMOCRM_PLUGIN_LOG_FILE)) {
            return;
        }

        unlink(WC_AMOCRM_PLUGIN_LOG_FILE);

        add_action('admin_notices', function () {
            echo sprintf(
                '<div class="updated notice notice-success is-dismissible"><p>%1$s</p></div>',
                esc_html__('Log file has been cleared successfully.', 'wc-amocrm-integration')
            );
        });
    }
}
