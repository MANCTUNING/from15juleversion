...
function xfavi_business($postId, $product, $data, $numFeed = '1') {	
 xfavi_error_log('FEED № '.$numFeed.'; Стартовала xfavi_business. $postId = '.$postId.'; Файл: business.php; Строка: '.__LINE__, 0);	
 $result_xml = '';
 ...
 if (get_post_meta($postId, '_xfavi_goods_type', true) === '' || get_post_meta($postId, '_xfavi_goods_type', true) === 'default') {
	...
	if (get_term_meta($data['catid'], 'xfavi_default_goods_type', true) == '') {
		xfavi_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к отсутствует GoodsType; Файл: business.php; Строка: '.__LINE__, 0); return $result_xml;
	} else {		
...
// $data_xml .= $data['result_xml_desc'];
 ...
 if (is_array($result_main_part)) {
	xfavi_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.'. Успешно приняты данные от xfavi_main_part; Файл: business.php; Строка: '.__LINE__, 0);
	return $result_main_part;
 } else {
	xfavi_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к xfavi_main_part вернул string; Файл: business.php; Строка: '.__LINE__, 0); 
	return $result_xml;
 }
} // end function business($postId) {
?>